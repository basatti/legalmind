...
From inside `reason`, `_after_reason` reads `should_continue`: True loops
back to `retrieve` for another pass, False falls through to `answer`. The
loop is capped by `MAX_ITERATIONS = 3` in the reason node itself, the
...
`MAX_ITERATIONS` so this can never loop forever.
...
reason/retrieve loop is gated behind the `RAG_MULTI_STEP_ENABLED` environment
...
- The reason node and the retrieve/reason loop stay wired in the graph
  (so there's one graph shape to reason about, not two) but are unreachable.

Turning the loop on is one env var (`RAG_MULTI_STEP_ENABLED=true`). The loop
...
- read by `retrieve` on every pass
  (`retrieval_service.retrieve(query, within=state.authorized)`),
...
- `should_continue` is read once, right after `reason` runs, by
  `_after_reason`, never by `reason` itself on a later call. `iterations` is
  the reason node's own counter: it reads `state.iterations` on every pass and
  writes back an incremented value, so each pass decides fresh rather than
  from what it decided last time.
