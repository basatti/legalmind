...
multi-step one reasons first â€” decomposing the question before any search runs,
...
from graph.nodes import answer, cite, make_reason_node, make_route_node, retrieve
...
Context is `Any` because no context schema is declared â€” LangGraph's runtime
...
def _after_route(state: GraphState) -> Route:
    ...
    it, so this is unreachable in practice â€” but the fallback is the cheap path
...
def build_graph(llm: LLMProvider) -> RagGraph:
    ...
    graph against a fake. Compiling is otherwise pure â€” no database, no
    network, no credentials â€” and the model is not called until a run reaches
    the route node.
    """
