...
def register_and_login(client, session, email, role) -> int:
    """Create a user with a specific role directly in the DB, log them in,
    return their id."""
...
def test_attorney_can_read_assigned_case(client, session):
    ...
    user_id = register_and_login(client, session, "amy@example.com", Role.ATTORNEY)
...
def test_paralegal_cannot_read_unassigned_case(client, session):
    case = make_case(session, title="Unassigned Case")
    register_and_login(client, session, "priya@example.com", Role.PARALEGAL)
...
def test_paralegal_can_read_assigned_case(client, session):
    case = make_case(session, title="Assigned Case")
    user_id = register_and_login(client, session, "priya@example.com", Role.PARALEGAL)
    assign(session, user_id, case.id)
...
def test_partner_can_read_unassigned_case(client, session):
    case = make_case(session, title="Unassigned Case")
    register_and_login(client, session, "pete@example.com", Role.PARTNER)
...
def test_admin_can_read_unassigned_case(client, session):
    case = make_case(session, title="Unassigned Case")
    register_and_login(client, session, "alice@example.com", Role.ADMIN)
